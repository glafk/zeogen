##################################################################
# Implementation adapted from https://github.com/khornlund/pytorch-balanced-sampler/blob/master/pytorch_balanced_sampler/sampler.py
##################################################################
...
class SamplerFactory:
    ...
    def get(self, class_idxs, batch_size, n_batches, alpha, kind):
        ...
        Parameters
        ----------
        class_idxs : dict with classes as keys and 
            Dict of classes and lists of samples belonging to a class
            e.g. {'a', : [2,3,4], 'b': [1,5,6]}
    ...
    def _weight_classes(self, class_idxs, alpha):
        class_sizes = np.asarray([len(idxs) for _, idxs in class_idxs.items()])
        n_samples = class_sizes.sum()
        n_classes = len(class_idxs.keys())
...
class WeightedRandomBatchSampler(BatchSampler):
    ...
    class_weights : `numpy.array(int)`
        dict with classes as keys and number of samples of each class to include in each batch as values.

    class_idxs : dict of keys and list of integers
    ...
    def __init__(self, class_weights, class_idxs, batch_size, n_batches):
        self.sample_idxs = []
        for _, idxs in class_idxs.items():
